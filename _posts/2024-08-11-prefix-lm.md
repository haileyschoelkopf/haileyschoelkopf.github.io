...
PrefixLM also introduces a hyperparameter that must be fiddled with ($l \in [1, L]$ determining that $x_{<l}$ is treated as the bidirectional-attention "prefix") to either get the best output quality or to not push the model out-of-distribution from the $l$ value used during training. This is not a problem Causal LM faces!
...
This allows the $$Q$$, $$K^T$$, and $$V$$ matmuls to be re-associated, letting us avoid the $$O(L^2)$$ complexity of softmax attention by never producing $$(\phi(Q)\phi(K)^T) \in \mathbb{R}^{L \times L}$$. 
...
However, when we perform causal language modeling, we introduce a mask $M$ [TODO: describe M] that prevents tokens from being affected by *future* tokens in the sequence:
...
For each chunk, we compute the starting state for that chunk, reusing the state computed for the chunk prior:
...
It's not especially clear that one would want a data-dependent update like this when attending to inputs bidirectionally though--maybe we can get away with data-independent updates when attending to our prefix, but include this gating when attending causally to the output. Hydra<d-cite key="hwang2024hydrabidirectionalstatespace"></d-cite> may offer some hints here.
